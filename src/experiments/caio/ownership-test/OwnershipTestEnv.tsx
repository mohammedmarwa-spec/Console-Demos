...
import type { CloudProviderId } from '../../../screens/serviceRegions'
import {
  ONBOARDING_CHECKABLE_CARD_CSS,
  ONBOARDING_CHECKABLE_CARD_RING_CSS,
} from '../../../screens/playground/playgroundShared'
import { OnboardingTestEnvShell } from '../../../screens/playground/OnboardingTestEnvShell'
import {
...
} from '../../../screens/playground/testEnvServicesCatalog'
import type { OnboardingTestEnvCreatePayload, OnboardingTestEnvProps } from '../../../screens/playground/OnboardingTestEnv'
import { TEAM_TEST_ENV_CONTEXT } from '../../../mocks/console-context'
import {
  DEFAULT_WORKSPACE_OWNERSHIP_ID,
  WORKSPACE_OWNERSHIP_OPTIONS,
  type WorkspaceOwnershipId,
  type WorkspaceOwnershipOption,
...
function CloudProviderStack({ size = DENSE_SECONDARY_BUTTON_SIZE }: { size?: number }) {
...
function WorkspaceOwnershipCard({ option }: { option: WorkspaceOwnershipOption }) {
  return (
    <Card
      fullWidth
      checkable
...
WorkspaceOwnershipCard.displayName = 'WorkspaceOwnershipCard'
...
export type OwnershipTestEnvProps = OnboardingTestEnvProps

export function OwnershipTestEnv({
  userInitials,
  defaultProjectName,
  onSkip,
  onCreate,
  onCustomizePlan,
}: OwnershipTestEnvProps) {
  const [projectName, setProjectName] = useState(defaultProjectName)
  const [location, setLocation] = useState<TestEnvLocationId>('finland')
  const [workspaceOwnershipId, setWorkspaceOwnershipId] = useState<WorkspaceOwnershipId>(
    DEFAULT_WORKSPACE_OWNERSHIP_ID,
  )
  ...
  const selectedWorkspace = useMemo(
    () =>
      WORKSPACE_OWNERSHIP_OPTIONS.find((option) => option.id === workspaceOwnershipId) ??
      WORKSPACE_OWNERSHIP_OPTIONS[0],
  ...
  function handleCreate() {
    const payload: OnboardingTestEnvCreatePayload = {
          ...
          <Box style={{ display: 'flex', flexDirection: 'column', gap: 24, minWidth: 0 }}>
            <Section title="Workspace ownership">
              <Box className="onboarding-checkable-cards">
                <Card.Group
                  checked={workspaceOwnershipId}
                  onCheckedChange={({ value }) =>
                    setWorkspaceOwnershipId((value as WorkspaceOwnershipId) ?? DEFAULT_WORKSPACE_OWNERSHIP_ID)
                  }
                >
                  <Box
                    style={{
                  ...
                  >
                    {WORKSPACE_OWNERSHIP_OPTIONS.map((option) => (
                      <WorkspaceOwnershipCard key={option.id} option={option} />
                    ))}
                  </Box>
                </Card.Group>
              </Box>
            ...
            <Section title="Project details">
              ...
              >
                <Input
                  ...
                  description={selectedWorkspace.projectDescription}
              ...
              <Box style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                >
                  <Box style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                    <Typography.DefaultStrong>Workspace</Typography.DefaultStrong>
                    <StatusChip text={selectedWorkspace.orgSublabel} status="neutral" dense />
                  </Box>
                  <PlanDetailRow label="Organization" value={selectedWorkspace.orgName} />
                  <PlanDetailRow
                    label="Owned by"
                    value={selectedWorkspace.ownerLabel}
                    info={
                      workspaceOwnershipId === 'organization'
                        ? `Members of ${TEAM_TEST_ENV_CONTEXT.orgName} with admin role can manage this project`
                        : 'Only you can manage billing and access for this workspace'
                    }
                  />
                  <PlanDetailRow
                    label="Billing"
                    value={selectedWorkspace.billingNote}
                    info="Ownership affects who receives invoices after the trial ends"
                  />
                </Box>
...
OwnershipTestEnv.displayName = 'OwnershipTestEnv'
