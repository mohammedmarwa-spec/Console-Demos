...
export function ProjectListContent() {
  ...
  const [pendingDemoOfferId, setPendingDemoOfferId] = useState<string | null>(null)
  const [skippedDemoServiceId, setSkippedDemoServiceId] = useState<string | null>(null)
  ...
  const skippedDemoService = skippedDemoServiceId
    ? services.find((service) => service.id === skippedDemoServiceId) ?? null
    : null

  const pendingDemoService = pendingDemoOfferId
    ? services.find((service) => service.id === pendingDemoOfferId) ?? null
    : null

  function skipDemoOffer() {
    setPendingDemoOfferId((current) => {
      if (current) setSkippedDemoServiceId(current)
      return null
    })
  }

  function startDemo(serviceId: string) {
    setPendingDemoOfferId(null)
    setSkippedDemoServiceId(null)
    openNodeView(serviceId, 'vector-search-demo')
  }
  ...
  return (
    <Box style={{ display: 'flex', flex: 1, minHeight: 0, overflow: 'hidden' }}>
      ...
      >
        ...
        {pendingDemoOfferId ? null : skippedDemoService ? (
          <Box style={{ marginBottom: 16 }}>
            <Alert.Banner
              type="information"
              title="Vector search demo is ready"
              action={{
                text: 'Start demo',
                onClick: () => startDemo(skippedDemoService.id),
              }}
              onDismiss={() => setSkippedDemoServiceId(null)}
            >
              {skippedDemoService.serviceName} includes a pre-configured vector search demo. You can
              also open it later from the service sidebar.
            </Alert.Banner>
          </Box>
        ) : lastCreatedName ? (
          <Box style={{ marginBottom: 16 }}>
            <CreateSuccessBanner
              serviceName={lastCreatedName}
              onDismiss={() => setLastCreatedName(null)}
            />
          </Box>
        ) : lastUpgradedName ? (
          <Box style={{ marginBottom: 16 }}>
      ...
      <Dialog
        type="confirmation"
        title="Start the vector search demo?"
        open={pendingDemoService !== null}
        onClose={skipDemoOffer}
        primaryAction={{
          text: 'Start demo',
          onClick: () => {
            if (pendingDemoOfferId) startDemo(pendingDemoOfferId)
          },
        }}
        secondaryAction={{ text: 'Skip for now', onClick: skipDemoOffer }}
      >
        This Free or Developer OpenSearch service includes a pre-configured vector search demo. You
        can skip and start it later from the service sidebar or the banner on this page.
      </Dialog>
...
function CreateSuccessBanner({
  ...
  return (
    ...
    >
      <Typography.Small color="intense">
        {serviceName} is being created. It will appear in the list as Running.
      </Typography.Small>
      <Box style={{ flexShrink: 0 }}>
        <Button.Ghost dense type="button" onClick={onDismiss}>
          Dismiss
...
CreateSuccessBanner.displayName = 'CreateSuccessBanner'
