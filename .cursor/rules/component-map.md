...
- **Must** query Storybook MCP (`user-storybook` / `aiven-storybook`) before using an Aquarium component. In-repo screens are not the source of truth. If the MCP is missing, stop — see `.cursor/rules/aquarium-storybook-mcp.mdc`.
